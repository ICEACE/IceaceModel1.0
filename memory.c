...
void unittest_firm_init_employment_FirmInitEmployment_FirmInitBalanceSheet()
{
	...
	rc = MB_Iterator_CreateFiltered(b_jpoffice_firm_employee, &i_jpoffice_firm_employee, &FLAME_filter_firm_firm_init_employment_FirmInitEmployment_FirmInitBalanceSheet_jpoffice_firm_employee, current_xmachine_firm);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'jpoffice_firm_employee'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'jpoffice_firm_employee' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_firm_consumption_receive_sales_FirmReceiveSales_FirmConsumptionPrice()
{
	...
	rc = MB_Iterator_CreateFiltered(b_sold, &i_sold, &FLAME_filter_firm_firm_consumption_receive_sales_FirmReceiveSales_FirmConsumptionPrice_sold, current_xmachine_firm);
	...
	//return firm_consumption_receive_sales();
}
...
void unittest_firm_labour_job_offer_stage2_FirmLabourStage2End_FirmConsumptionStart()
{
	...
	rc = MB_Iterator_CreateFiltered(b_job_match_stage2, &i_job_match_stage2, &FLAME_filter_firm_firm_labour_job_offer_stage2_FirmLabourStage2End_FirmConsumptionStart_job_match_stage2, current_xmachine_firm);
...
void unittest_firm_credit_borrow_loans_1_FirmCreditLoanBorrow1_FirmCreditLoanBorrow2()
{
	...
	rc = MB_Iterator_CreateFiltered(b_bank_firm_loan_acknowledge_1, &i_bank_firm_loan_acknowledge_1, &FLAME_filter_firm_firm_credit_borrow_loans_1_FirmCreditLoanBorrow1_FirmCreditLoanBorrow2_bank_firm_loan_acknowledge_1, current_xmachine_firm);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_firm_loan_acknowledge_1'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_firm_loan_acknowledge_1' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_firm_credit_borrow_loans_2_FirmCreditLoanBorrow2_FirmCreditIlliquidityStatus()
{
	...
	rc = MB_Iterator_CreateFiltered(b_bank_firm_loan_acknowledge_2, &i_bank_firm_loan_acknowledge_2, &FLAME_filter_firm_firm_credit_borrow_loans_2_FirmCreditLoanBorrow2_FirmCreditIlliquidityStatus_bank_firm_loan_acknowledge_2, current_xmachine_firm);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_firm_loan_acknowledge_2'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_firm_loan_acknowledge_2' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_firm_credit_insolvency_bankruptcy_FirmCreditInsolvencyCheck_FirmProductionStart()
{
	...
	//return firm_credit_insolvency_bankruptcy();
}
...
void unittest_firm_credit_check_tax_rate_FirmCreditCheckTaxRate_end()
{
	...
	//return firm_credit_check_tax_rate();
}
...
void unittest_household_init_employment_HouseholdInitEmployment_HouseholdInitBalanceSheet()
{
	...
	rc = MB_Iterator_CreateFiltered(b_jpoffice_household_employer, &i_jpoffice_household_employer, &FLAME_filter_household_household_init_employment_HouseholdInitEmployment_HouseholdInitBalanceSheet_jpoffice_household_employer, current_xmachine_household);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'jpoffice_household_employer'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'jpoffice_household_employer' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return household_init_employment();
}
...
void unittest_household_init_balancesheet_HouseholdInitBalanceSheet_HouseholdLoop()
{
	...
	//return household_init_balancesheet();
}
...
void unittest_household_consumption_recieve_goods_HHConsumptionRecieveGoods_HHConsumptionMonthly()
{
	...
	rc = MB_Iterator_CreateFiltered(b_bought, &i_bought, &FLAME_filter_household_household_consumption_recieve_goods_HHConsumptionRecieveGoods_HHConsumptionMonthly_bought, current_xmachine_household);
...
void unittest_household_consumption_trace_cpi_HHConsumptionMonthly_HHHousingStart()
{
	...
	//return household_consumption_trace_cpi();
}
...
void unittest_household_labour_turnover_HHLabourStage2_HHConsumptionStart()
{
	...
	rc = MB_Iterator_CreateFiltered(b_job_match_stage1, &i_job_match_stage1, &FLAME_filter_household_household_labour_turnover_HHLabourStage2_HHConsumptionStart_job_match_stage1, current_xmachine_household);
...
void unittest_household_labour_employment_HHLabourStage1Placement_HHConsumptionStart()
{
	...
	rc = MB_Iterator_CreateFiltered(b_job_match_stage2, &i_job_match_stage2, &FLAME_filter_household_household_labour_employment_HHLabourStage1Placement_HHConsumptionStart_job_match_stage2, current_xmachine_household);
...
void unittest_household_labour_receive_wage_HHLabourPayroll_HHLabourEmploymentStatus()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_household_wage_payment, &i_firm_household_wage_payment, &FLAME_filter_household_household_labour_receive_wage_HHLabourPayroll_HHLabourEmploymentStatus_firm_household_wage_payment, current_xmachine_household);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_household_wage_payment'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_household_wage_payment' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return household_labour_receive_wage();
}
...
void unittest_equityfund_credit_compute_income_statement_EFCreditIncomeStatement_EFCreditBalanceSheet()
{
	...
	//return equityfund_credit_compute_income_statement();
}
...
void unittest_equityfund_credit_check_tax_rate_EFCreditPolicyChecks_end()
{
	...
	//return equityfund_credit_check_tax_rate();
}
...
void unittest_bank_init_loans_BankInitLoans_BankInitMortgages()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_init_loans, &i_firm_bank_init_loans, &FLAME_filter_bank_bank_init_loans_BankInitLoans_BankInitMortgages_firm_bank_init_loans, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_init_loans'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_init_loans' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_init_mortgages_BankInitMortgages_BankInitDeposits()
{
	...
	rc = MB_Iterator_CreateFiltered(b_household_bank_init_mortgages, &i_household_bank_init_mortgages, &FLAME_filter_bank_bank_init_mortgages_BankInitMortgages_BankInitDeposits_household_bank_init_mortgages, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'household_bank_init_mortgages'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'household_bank_init_mortgages' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_init_deposits_BankInitDeposits_BankInitBalanceSheet()
{
	...
	rc = MB_Iterator_CreateFiltered(b_household_bank_init_deposit, &i_household_bank_init_deposit, &FLAME_filter_bank_bank_init_deposits_BankInitDeposits_BankInitBalanceSheet_household_bank_init_deposit, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'household_bank_init_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'household_bank_init_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_init_deposit, &i_firm_bank_init_deposit, &FLAME_filter_bank_bank_init_deposits_BankInitDeposits_BankInitBalanceSheet_firm_bank_init_deposit, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_init_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_init_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_update_deposits_BankUpdateDeposits_BankCreditPolicyChecks()
{
	...
	rc = MB_Iterator_CreateFiltered(b_household_bank_update_deposit, &i_household_bank_update_deposit, &FLAME_filter_bank_bank_update_deposits_BankUpdateDeposits_BankCreditPolicyChecks_household_bank_update_deposit, current_xmachine_bank);
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_update_deposit, &i_firm_bank_update_deposit, &FLAME_filter_bank_bank_update_deposits_BankUpdateDeposits_BankCreditPolicyChecks_firm_bank_update_deposit, current_xmachine_bank);
...
void unittest_bank_credit_process_loan_requests_1_BankCreditRequest1_BankCreditRequest2()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_loan_request_1, &i_firm_bank_loan_request_1, &FLAME_filter_bank_bank_credit_process_loan_requests_1_BankCreditRequest1_BankCreditRequest2_firm_bank_loan_request_1, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_loan_request_1'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_loan_request_1' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_credit_process_loan_requests_2_BankCreditRequest2_BankCreditLoanWriteOffs()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_loan_request_2, &i_firm_bank_loan_request_2, &FLAME_filter_bank_bank_credit_process_loan_requests_2_BankCreditRequest2_BankCreditLoanWriteOffs_firm_bank_loan_request_2, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_loan_request_2'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_loan_request_2' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_credit_recieve_loan_writeoffs_BankCreditLoanWriteOffs_BankCreditNewEntrants()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_insolvent_account, &i_firm_bank_insolvent_account, &FLAME_filter_bank_bank_credit_recieve_loan_writeoffs_BankCreditLoanWriteOffs_BankCreditNewEntrants_firm_bank_insolvent_account, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_insolvent_account'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_insolvent_account' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_credit_collect_loan_interests_BankCreditLoanInterests_BankHousingMarket()
{
	...
	rc = MB_Iterator_CreateFiltered(b_firm_bank_interest_on_loan, &i_firm_bank_interest_on_loan, &FLAME_filter_bank_bank_credit_collect_loan_interests_BankCreditLoanInterests_BankHousingMarket_firm_bank_interest_on_loan, current_xmachine_bank);
	
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bank_interest_on_loan'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_bank_interest_on_loan' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_bank_credit_check_interest_rate_BankCreditPolicyChecks_end()
{
	...
	//return bank_credit_check_interest_rate();
}
...
void unittest_government_collect_centralbank_profit_GovernmentFiscalPolicy_GovernmentIncomeStatement()
{
	...
	//return government_collect_centralbank_profit();
}
...
void unittest_centralbank_init_balancesheet_CentralBankInitBalanceSheet_CentralBankLoop()
{
	...
	rc = MB_Iterator_Create(b_bank_centralbank_init_debt, &i_bank_centralbank_init_debt);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_centralbank_init_debt'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_centralbank_init_debt' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_bank_centralbank_init_deposit, &i_bank_centralbank_init_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_centralbank_init_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_centralbank_init_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_fund_centralbank_init_deposit, &i_fund_centralbank_init_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'fund_centralbank_init_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'fund_centralbank_init_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_gov_centralbank_init_deposit, &i_gov_centralbank_init_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'gov_centralbank_init_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'gov_centralbank_init_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return centralbank_init_balancesheet();
}
...
void unittest_centralbank_update_price_indices_CentralBankConsumptionPrices_CentralBankHousing()
{
	...
	//return centralbank_update_price_indices();
}
...
void unittest_centralbank_collect_interest_payments_CentralBankCredit_CentralBankDebtRequests()
{
	...
	//return centralbank_collect_interest_payments();
}
...
void unittest_centralbank_process_debt_requests_CentralBankDebtRequests_CentralBankConsumption()
{
	...
	rc = MB_Iterator_Create(b_bank_centralbank_debt_request, &i_bank_centralbank_debt_request);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_centralbank_debt_request'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_centralbank_debt_request' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_bank_centralbank_update_deposit, &i_bank_centralbank_update_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_centralbank_update_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_centralbank_update_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_centralbank_process_government_requests_CentralBankGovernmentRequests_CentralBankBalanceSheet()
{
	...
	rc = MB_Iterator_Create(b_gov_centralbank_debt_request, &i_gov_centralbank_debt_request);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'gov_centralbank_debt_request'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'gov_centralbank_debt_request' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_gov_centralbank_debt_payment, &i_gov_centralbank_debt_payment);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'gov_centralbank_debt_payment'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'gov_centralbank_debt_payment' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_gov_centralbank_update_deposit, &i_gov_centralbank_update_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'gov_centralbank_update_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'gov_centralbank_update_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return centralbank_process_government_requests();
}
...
void unittest_centralbank_do_balance_sheet_CentralBankBalanceSheet_CentralBankUpdateInterestRate()
{
	...
	rc = MB_Iterator_Create(b_fund_centralbank_update_deposit, &i_fund_centralbank_update_deposit);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'fund_centralbank_update_deposit'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'fund_centralbank_update_deposit' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void unittest_jpoffice_init_employment_JPOfficeInitEmployment_JPOfficeLoop()
{
	...
	rc = MB_Iterator_Create(b_household_jpoffice_id, &i_household_jpoffice_id);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'household_jpoffice_id'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'household_jpoffice_id' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	rc = MB_Iterator_Create(b_firm_jpoffice_id, &i_firm_jpoffice_id);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'firm_jpoffice_id'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'firm_jpoffice_id' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
	...
	//return jpoffice_init_employment();
}
...
void unittest_reagency_check_interest_rate_REAgencyCreditPolicyChecks_end()
{
	...
	//return reagency_check_interest_rate();
}
...
void unittest_reagency_housing_process_REAgencyHousingMarket_REAgencyHousingSummary()
{
	...
	rc = MB_Iterator_Create(b_bank_reagency_credibility, &i_bank_reagency_credibility);
	#ifdef ERRCHECK
	if (rc != MB_SUCCESS)
	{
	   fprintf(stderr, "ERROR: Could not create Iterator for 'bank_reagency_credibility'\n");
	   switch(rc) {
	       case MB_ERR_INVALID:
	           fprintf(stderr, "\t reason: 'bank_reagency_credibility' board is invalid\n");
	           break;
	       case MB_ERR_LOCKED:
...
void free_messages()
{
	int rc;
	    ...
	    rc = MB_Clear(b_firm_bank_init_loans);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_init_loans' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_init_loans' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_bank_init_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_init_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_household_bank_init_mortgages);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'household_bank_init_mortgages' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'household_bank_init_mortgages' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_household_bank_init_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'household_bank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'household_bank_init_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_centralbank_init_debt);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_centralbank_init_debt' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_centralbank_init_debt' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_centralbank_init_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_centralbank_init_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_centralbank_update_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_centralbank_update_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_fund_centralbank_init_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'fund_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'fund_centralbank_init_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_fund_centralbank_update_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'fund_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'fund_centralbank_update_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_gov_centralbank_init_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'gov_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'gov_centralbank_init_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_gov_centralbank_update_deposit);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'gov_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'gov_centralbank_update_deposit' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_household_jpoffice_id);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'household_jpoffice_id' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'household_jpoffice_id' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_jpoffice_household_employer);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'jpoffice_household_employer' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'jpoffice_household_employer' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_jpoffice_id);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_jpoffice_id' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_jpoffice_id' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_jpoffice_firm_employee);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'jpoffice_firm_employee' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'jpoffice_firm_employee' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_household_wage_payment);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_household_wage_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_household_wage_payment' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_bank_loan_request_1);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_loan_request_1' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_loan_request_1' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_bank_loan_request_2);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_loan_request_2' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_loan_request_2' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_firm_loan_acknowledge_1);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_firm_loan_acknowledge_1' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_firm_loan_acknowledge_1' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_firm_loan_acknowledge_2);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_firm_loan_acknowledge_2' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_firm_loan_acknowledge_2' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_centralbank_debt_request);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_centralbank_debt_request' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_centralbank_debt_request' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_bank_interest_on_loan);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_interest_on_loan' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_interest_on_loan' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_firm_bank_insolvent_account);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'firm_bank_insolvent_account' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'firm_bank_insolvent_account' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_gov_centralbank_debt_request);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'gov_centralbank_debt_request' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'gov_centralbank_debt_request' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_gov_centralbank_debt_payment);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'gov_centralbank_debt_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'gov_centralbank_debt_payment' board is invalid\n");
	               break;
	    ...
	    rc = MB_Clear(b_bank_reagency_credibility);
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not clear 'bank_reagency_credibility' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: 'bank_reagency_credibility' board is invalid\n");
	               break;
...
int rc;
	...
	FLAME_m_firm_bank_init_loans_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_init_loans, sizeof(m_firm_bank_init_loans));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_init_loans' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_bank_init_deposit_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_init_deposit, sizeof(m_firm_bank_init_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_household_bank_init_mortgages_composite_params = NULL;

	    rc = MB_Create(&b_household_bank_init_mortgages, sizeof(m_household_bank_init_mortgages));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'household_bank_init_mortgages' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_household_bank_init_deposit_composite_params = NULL;

	    rc = MB_Create(&b_household_bank_init_deposit, sizeof(m_household_bank_init_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'household_bank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_init_debt_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_init_debt, sizeof(m_bank_centralbank_init_debt));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_init_debt' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_init_deposit_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_init_deposit, sizeof(m_bank_centralbank_init_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_update_deposit_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_update_deposit, sizeof(m_bank_centralbank_update_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_fund_centralbank_init_deposit_composite_params = NULL;

	    rc = MB_Create(&b_fund_centralbank_init_deposit, sizeof(m_fund_centralbank_init_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'fund_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_fund_centralbank_update_deposit_composite_params = NULL;

	    rc = MB_Create(&b_fund_centralbank_update_deposit, sizeof(m_fund_centralbank_update_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'fund_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_gov_centralbank_init_deposit_composite_params = NULL;

	    rc = MB_Create(&b_gov_centralbank_init_deposit, sizeof(m_gov_centralbank_init_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'gov_centralbank_init_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_gov_centralbank_update_deposit_composite_params = NULL;

	    rc = MB_Create(&b_gov_centralbank_update_deposit, sizeof(m_gov_centralbank_update_deposit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'gov_centralbank_update_deposit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_household_jpoffice_id_composite_params = NULL;

	    rc = MB_Create(&b_household_jpoffice_id, sizeof(m_household_jpoffice_id));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'household_jpoffice_id' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_jpoffice_household_employer_composite_params = NULL;

	    rc = MB_Create(&b_jpoffice_household_employer, sizeof(m_jpoffice_household_employer));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'jpoffice_household_employer' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_jpoffice_id_composite_params = NULL;

	    rc = MB_Create(&b_firm_jpoffice_id, sizeof(m_firm_jpoffice_id));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_jpoffice_id' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_jpoffice_firm_employee_composite_params = NULL;

	    rc = MB_Create(&b_jpoffice_firm_employee, sizeof(m_jpoffice_firm_employee));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'jpoffice_firm_employee' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_household_wage_payment_composite_params = NULL;

	    rc = MB_Create(&b_firm_household_wage_payment, sizeof(m_firm_household_wage_payment));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_household_wage_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_bank_loan_request_1_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_loan_request_1, sizeof(m_firm_bank_loan_request_1));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_loan_request_1' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_bank_loan_request_2_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_loan_request_2, sizeof(m_firm_bank_loan_request_2));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_loan_request_2' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_firm_loan_acknowledge_1_composite_params = NULL;

	    rc = MB_Create(&b_bank_firm_loan_acknowledge_1, sizeof(m_bank_firm_loan_acknowledge_1));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_firm_loan_acknowledge_1' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_firm_loan_acknowledge_2_composite_params = NULL;

	    rc = MB_Create(&b_bank_firm_loan_acknowledge_2, sizeof(m_bank_firm_loan_acknowledge_2));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_firm_loan_acknowledge_2' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_debt_request_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_debt_request, sizeof(m_bank_centralbank_debt_request));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_debt_request' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_bank_interest_on_loan_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_interest_on_loan, sizeof(m_firm_bank_interest_on_loan));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_interest_on_loan' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_firm_bank_insolvent_account_composite_params = NULL;

	    rc = MB_Create(&b_firm_bank_insolvent_account, sizeof(m_firm_bank_insolvent_account));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'firm_bank_insolvent_account' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_interest_payment_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_interest_payment, sizeof(m_bank_centralbank_interest_payment));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_interest_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_centralbank_debt_payment_composite_params = NULL;

	    rc = MB_Create(&b_bank_centralbank_debt_payment, sizeof(m_bank_centralbank_debt_payment));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_centralbank_debt_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_gov_centralbank_debt_request_composite_params = NULL;

	    rc = MB_Create(&b_gov_centralbank_debt_request, sizeof(m_gov_centralbank_debt_request));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'gov_centralbank_debt_request' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_gov_centralbank_debt_payment_composite_params = NULL;

	    rc = MB_Create(&b_gov_centralbank_debt_payment, sizeof(m_gov_centralbank_debt_payment));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'gov_centralbank_debt_payment' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_centralbank_government_profit_composite_params = NULL;

	    rc = MB_Create(&b_centralbank_government_profit, sizeof(m_centralbank_government_profit));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'centralbank_government_profit' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	/* Initialise message sync composite params as NULL */
	FLAME_m_bank_reagency_credibility_composite_params = NULL;

	    rc = MB_Create(&b_bank_reagency_credibility, sizeof(m_bank_reagency_credibility));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'bank_reagency_credibility' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	               fprintf(stderr, "\t reason: Invalid message size\n");
	               break;
	...
	firm_FirmCreditCheckTaxRate_state = init_firm_state();

	firm_end_state = init_firm_state();

	firm_FirmCreditInsolvencyCheck_state = init_firm_state();
	...
	firm_FirmCreditLiquidityRecheck_state = init_firm_state();
	...
	firm_FirmReceiveSales_state = init_firm_state();

	firm_FirmConsumptionMarket_state = init_firm_state();

	firm_FirmHousingStart_state = init_firm_state();

	firm_FirmConsumptionStart_state = init_firm_state();

	firm_FirmCreditPolicyChecks_state = init_firm_state();

	firm_FirmUpdateDeposits_state = init_firm_state();

	firm_FirmCreditStart_state = init_firm_state();

	firm_FirmInitBalanceSheet_state = init_firm_state();

	firm_FirmInitEmployment_state = init_firm_state();

	firm_FirmLoop_state = init_firm_state();

	firm_FirmInitPostId_state = init_firm_state();
	...
	household_HHCreditCheckTaxRate_state = init_household_state();

	household_HHCreditUpdateAnnuity_state = init_household_state();
	...
	household_HHConsumptionMonthly_state = init_household_state();

	household_HHConsumptionWeekly_state = init_household_state();

	household_HHConsumptionStart_state = init_household_state();

	household_HHCreditPolicyChecks_state = init_household_state();

	household_HouseholdUpdateDeposits_state = init_household_state();

	household_HHCreditStart_state = init_household_state();

	household_HouseholdInitBalanceSheet_state = init_household_state();

	household_HouseholdInitEmployment_state = init_household_state();

	household_HouseholdLoop_state = init_household_state();

	household_HouseholdInitPostId_state = init_household_state();

	household_start_state = init_household_state();

	equityfund_EFCreditPolicyChecks_state = init_equityfund_state();
	...
	equityfund_EFCreditIncomeStatement_state = init_equityfund_state();

	equityfund_EFLabourMarket_state = init_equityfund_state();

	equityfund_EquityFundCredit_state = init_equityfund_state();

	equityfund_EquityFundLoop_state = init_equityfund_state();
	...
	bank_BankCreditPolicyChecks_state = init_bank_state();

	bank_BankUpdateDeposits_state = init_bank_state();

	bank_BankCreditStart_state = init_bank_state();

	bank_BankInitBalanceSheet_state = init_bank_state();

	bank_BankInitDeposits_state = init_bank_state();

	bank_BankInitMortgages_state = init_bank_state();

	bank_BankLoop_state = init_bank_state();

	bank_BankInitLoans_state = init_bank_state();

	bank_start_state = init_bank_state();

	government_GovernmentUpdatePolicy_state = init_government_state();
	...
	government_GovernmentFiscalPolicy_state = init_government_state();

	government_GovernmentLabourStatus_state = init_government_state();

	government_GovernmentCredit_state = init_government_state();

	government_GovernmentLoop_state = init_government_state();

	government_start_state = init_government_state();

	centralbank_CentralBankUpdateInterestRate_state = init_centralbank_state();

	centralbank_CentralBankBalanceSheet_state = init_centralbank_state();

	centralbank_CentralBankGovernmentRequests_state = init_centralbank_state();

	centralbank_end_state = init_centralbank_state();

	centralbank_CentralBankDebtRequests_state = init_centralbank_state();

	centralbank_CentralBankPolicy_state = init_centralbank_state();

	centralbank_CentralBankLabour_state = init_centralbank_state();

	centralbank_CentralBankHousing_state = init_centralbank_state();

	centralbank_CentralBankConsumptionPrices_state = init_centralbank_state();
	...
	centralbank_CentralBankLoop_state = init_centralbank_state();

	centralbank_CentralBankInitBalanceSheet_state = init_centralbank_state();
	...
	jpoffice_JPOfficeLoop_state = init_jpoffice_state();

	jpoffice_JPOfficeInitEmployment_state = init_jpoffice_state();
	...
	reagency_REAgencyCreditPolicyChecks_state = init_reagency_state();
...
void copy_mortgage(mortgage * from, mortgage * to)
{
	...
	(*to).interestrate = (*from).interestrate;
	(*to).mtype = (*from).mtype;
}
...
xmachine_memory_firm * init_firm_agent()
{
	...
	current->capital_productivity = 0.0;
	current->capital_goods = 0;
	current->capital_goods_price = 0.0;
	current->production_current = 0;
	current->expected_sales = 0;
	...
	init_int_static_array(current->projects, 12);
	...
	current->isliquidshort = 0;
	current->hasloan = 0;
	current->hasinvestment = 0;
	current->isilliquid = 0;
...
void unittest_init_firm_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_firm->manager = 0;
		...
		current_xmachine_firm->capital_productivity = 0.0;
		current_xmachine_firm->capital_goods = 0;
		current_xmachine_firm->capital_goods_price = 0.0;
		current_xmachine_firm->production_current = 0;
		current_xmachine_firm->expected_sales = 0;
		current_xmachine_firm->production_plan = 0;
		current_xmachine_firm->unit_goods_price = 0.0;
		current_xmachine_firm->unit_cost = 0.0;
		current_xmachine_firm->day_of_month_production_completed = 0;
		current_xmachine_firm->unit_house_price = 0.0;
		init_int_static_array(current_xmachine_firm->projects, 12);
		...
		current_xmachine_firm->isliquidshort = 0;
		current_xmachine_firm->hasloan = 0;
		current_xmachine_firm->hasinvestment = 0;
		current_xmachine_firm->isilliquid = 0;
...
void free_firm_agents()
{
	...
	current_xmachine_firm_holder = firm_FirmReceiveSales_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmReceiveSales_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmReceiveSales_state->count = 0;
	current_xmachine_firm_holder = firm_FirmConsumptionMarket_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmConsumptionMarket_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmConsumptionMarket_state->count = 0;
	...
	current_xmachine_firm_holder = firm_FirmCreditPolicyChecks_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmCreditPolicyChecks_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmCreditPolicyChecks_state->count = 0;
	...
	current_xmachine_firm_holder = firm_FirmInitBalanceSheet_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmInitBalanceSheet_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmInitBalanceSheet_state->count = 0;
	current_xmachine_firm_holder = firm_FirmInitEmployment_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmInitEmployment_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmInitEmployment_state->count = 0;
	current_xmachine_firm_holder = firm_FirmLoop_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmLoop_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmLoop_state->count = 0;
	current_xmachine_firm_holder = firm_FirmInitPostId_state->agents;
	while(current_xmachine_firm_holder)
	{
		temp_xmachine_firm_holder = current_xmachine_firm_holder->next;
		free_firm_agent(current_xmachine_firm_holder, firm_FirmInitPostId_state);
		current_xmachine_firm_holder = temp_xmachine_firm_holder;
	}
	firm_FirmInitPostId_state->count = 0;
...
void free_firm_states()
{
	...
	free(firm_FirmCreditCheckTaxRate_state);
	free(firm_end_state);
	free(firm_FirmCreditInsolvencyCheck_state);
	free(firm_FirmCreditDividendPayments_state);
	free(firm_FirmCreditBalanceSheet_state);
	free(firm_FirmCreditBankruptcy_state);
	free(firm_FirmCreditLiquidityRecheck_state);
	...
	free(firm_FirmReceiveSales_state);
	free(firm_FirmConsumptionMarket_state);
	free(firm_FirmHousingStart_state);
	free(firm_FirmConsumptionStart_state);
	free(firm_FirmCreditPolicyChecks_state);
	free(firm_FirmUpdateDeposits_state);
	free(firm_FirmCreditStart_state);
	free(firm_FirmInitBalanceSheet_state);
	free(firm_FirmInitEmployment_state);
	free(firm_FirmLoop_state);
	free(firm_FirmInitPostId_state);
 ...
 * \param manager Variable for the X-machine memory.
 ...
 * \param capital_productivity Variable for the X-machine memory.
 * \param capital_goods Variable for the X-machine memory.
 * \param capital_goods_price Variable for the X-machine memory.
 * \param production_current Variable for the X-machine memory.
 * \param expected_sales Variable for the X-machine memory.
 * \param production_plan Variable for the X-machine memory.
 * \param unit_goods_price Variable for the X-machine memory.
 * \param unit_cost Variable for the X-machine memory.
 ...
 * \param isliquidshort Variable for the X-machine memory.
 * \param hasloan Variable for the X-machine memory.
 * \param hasinvestment Variable for the X-machine memory.
 * \param isilliquid Variable for the X-machine memory.
...
void add_firm_agent(int id, int bank_id, int isconstructor, int day_of_month_to_act, int isinsolvent, int it_no, int day_of_week_to_act, double average_goods_price, int_array * employees, int manager, double wage_offer, double average_wage, int no_employees, int vacancies, int employees_needed, int day_of_month_wages_paid, double labour_productivity, double capital_productivity, int capital_goods, double capital_goods_price, int production_current, int expected_sales, int production_plan, double unit_goods_price, double unit_cost, int day_of_month_production_completed, double unit_house_price, int projects[], double loans_interest_rate, double debt, int inventory, int sales, double revenues, double total_assets, double operating_costs, double labour_costs, double total_interest_payments, double dividends_paid, double dividends_to_be_paid, double retained_earnings, double net_earnings, double ebit, double equity, double liquidity, int isliquidshort, int hasloan, int hasinvestment, int isilliquid, double planned_investment_costs, double liquidity_need, loan loan_list[], double labour_tax_rate, double delta_housing_price)
{
	...
	current->capital_productivity = capital_productivity;
	current->capital_goods = capital_goods;
	current->capital_goods_price = capital_goods_price;
	current->production_current = production_current;
	current->expected_sales = expected_sales;
	...
	memcpy(current->projects, projects, 12*sizeof(int));
	...
	current->isliquidshort = isliquidshort;
	current->hasloan = hasloan;
	current->hasinvestment = hasinvestment;
	current->isilliquid = isilliquid;
...
xmachine_memory_household * init_household_agent()
{
	...
	current->wage = 0.0;
	current->ismanager = 0;
	current->government_benefits = 0.0;
	current->day_of_month_to_act = 0;
	...
	init_double_static_array(current->previous_wages, 3);
	init_double_static_array(current->previous_benefits, 3);
	current->labour_income = 0.0;
	current->total_assets = 0.0;
	current->housing_value = 0.0;
	current->expected_housing_payment = 0.0;
...
void unittest_init_household_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_household->government_benefits = 0.0;
		...
		init_double_static_array(current_xmachine_household->previous_wages, 3);
		init_double_static_array(current_xmachine_household->previous_benefits, 3);
		...
		current_xmachine_household->expected_housing_payment = 0.0;
		...
		current_xmachine_household->mortgage_choice = 0;
...
void free_household_agents()
{
	...
	current_xmachine_household_holder = household_HHCreditUpdateAnnuity_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HHCreditUpdateAnnuity_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HHCreditUpdateAnnuity_state->count = 0;
	...
	current_xmachine_household_holder = household_HHConsumptionMonthly_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HHConsumptionMonthly_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HHConsumptionMonthly_state->count = 0;
	...
	current_xmachine_household_holder = household_HHCreditPolicyChecks_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HHCreditPolicyChecks_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HHCreditPolicyChecks_state->count = 0;
	...
	current_xmachine_household_holder = household_HouseholdInitBalanceSheet_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HouseholdInitBalanceSheet_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HouseholdInitBalanceSheet_state->count = 0;
	current_xmachine_household_holder = household_HouseholdInitEmployment_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HouseholdInitEmployment_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HouseholdInitEmployment_state->count = 0;
	current_xmachine_household_holder = household_HouseholdLoop_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HouseholdLoop_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HouseholdLoop_state->count = 0;
	current_xmachine_household_holder = household_HouseholdInitPostId_state->agents;
	while(current_xmachine_household_holder)
	{
		temp_xmachine_household_holder = current_xmachine_household_holder->next;
		free_household_agent(current_xmachine_household_holder, household_HouseholdInitPostId_state);
		current_xmachine_household_holder = temp_xmachine_household_holder;
	}
	household_HouseholdInitPostId_state->count = 0;
...
void free_household_states()
{
	...
	free(household_HHCreditCheckTaxRate_state);
	free(household_HHCreditUpdateAnnuity_state);
	...
	free(household_HHConsumptionMonthly_state);
	free(household_HHConsumptionWeekly_state);
	free(household_HHConsumptionStart_state);
	free(household_HHCreditPolicyChecks_state);
	free(household_HouseholdUpdateDeposits_state);
	free(household_HHCreditStart_state);
	free(household_HouseholdInitBalanceSheet_state);
	free(household_HouseholdInitEmployment_state);
	free(household_HouseholdLoop_state);
	free(household_HouseholdInitPostId_state);
 ...
 * \param government_benefits Variable for the X-machine memory.
 * \param day_of_month_to_act Variable for the X-machine memory.
 ...
 * \param previous_benefits Variable for the X-machine memory.
 * \param labour_income Variable for the X-machine memory.
 * \param total_assets Variable for the X-machine memory.
 * \param housing_value Variable for the X-machine memory.
 * \param expected_housing_payment Variable for the X-machine memory.
...
void add_household_agent(int id, int bank_id, int it_no, int day_of_week_to_act, double weekly_consumption_budget, double mall_budget, double quarterly_price_change, int my_employer_id, double wage, int ismanager, double government_benefits, int day_of_month_to_act, int day_of_month_wage_recieved, double mortgages_interest_rate, double labour_tax_rate, mortgage_array * mortgages_list, double mortgages, double housing_payment, double equity, double housing_price, int housing_units, int n_shares, double liquidity, double capital_income, double previous_wages[], double previous_benefits[], double labour_income, double total_assets, double housing_value, double expected_housing_payment, int hmarket_role, double equity_ratio, double minimum_equity_ratio, double mortgage_costs[], double delta_housing_value, int mortgage_choice)
{
	...
	current->government_benefits = government_benefits;
	current->day_of_month_to_act = day_of_month_to_act;
	...
	memcpy(current->previous_wages, previous_wages, 3*sizeof(double));
	memcpy(current->previous_benefits, previous_benefits, 3*sizeof(double));
	current->labour_income = labour_income;
	current->total_assets = total_assets;
	current->housing_value = housing_value;
	current->expected_housing_payment = expected_housing_payment;
	...
	current->mortgage_choice = mortgage_choice;
}
...
void unittest_init_equityfund_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_equityfund->day_of_month_wages_paid = 0;
...
{
	current_xmachine_equityfund_holder = equityfund_EFCreditPolicyChecks_state->agents;
	while(current_xmachine_equityfund_holder)
	{
		temp_xmachine_equityfund_holder = current_xmachine_equityfund_holder->next;
		free_equityfund_agent(current_xmachine_equityfund_holder, equityfund_EFCreditPolicyChecks_state);
		current_xmachine_equityfund_holder = temp_xmachine_equityfund_holder;
	}
	equityfund_EFCreditPolicyChecks_state->count = 0;
	...
	current_xmachine_equityfund_holder = equityfund_EFCreditIncomeStatement_state->agents;
	while(current_xmachine_equityfund_holder)
	{
		temp_xmachine_equityfund_holder = current_xmachine_equityfund_holder->next;
		free_equityfund_agent(current_xmachine_equityfund_holder, equityfund_EFCreditIncomeStatement_state);
		current_xmachine_equityfund_holder = temp_xmachine_equityfund_holder;
	}
	equityfund_EFCreditIncomeStatement_state->count = 0;
	...
	current_xmachine_equityfund_holder = equityfund_EquityFundCredit_state->agents;
	while(current_xmachine_equityfund_holder)
	{
		temp_xmachine_equityfund_holder = current_xmachine_equityfund_holder->next;
		free_equityfund_agent(current_xmachine_equityfund_holder, equityfund_EquityFundCredit_state);
		current_xmachine_equityfund_holder = temp_xmachine_equityfund_holder;
	}
	equityfund_EquityFundCredit_state->count = 0;
	current_xmachine_equityfund_holder = equityfund_EquityFundLoop_state->agents;
	while(current_xmachine_equityfund_holder)
	{
		temp_xmachine_equityfund_holder = current_xmachine_equityfund_holder->next;
		free_equityfund_agent(current_xmachine_equityfund_holder, equityfund_EquityFundLoop_state);
		current_xmachine_equityfund_holder = temp_xmachine_equityfund_holder;
	}
	equityfund_EquityFundLoop_state->count = 0;
...
{
	free(equityfund_EFCreditPolicyChecks_state);
	...
	free(equityfund_EFCreditIncomeStatement_state);
	free(equityfund_EFLabourMarket_state);
	free(equityfund_EquityFundCredit_state);
	free(equityfund_EquityFundLoop_state);
...
void unittest_init_bank_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_bank->loans_start = 0.0;
		...
		current_xmachine_bank->interest_rate = 0.0;
...
void free_bank_agents()
{
	...
	current_xmachine_bank_holder = bank_BankCreditPolicyChecks_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankCreditPolicyChecks_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankCreditPolicyChecks_state->count = 0;
	...
	current_xmachine_bank_holder = bank_BankInitBalanceSheet_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankInitBalanceSheet_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankInitBalanceSheet_state->count = 0;
	current_xmachine_bank_holder = bank_BankInitDeposits_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankInitDeposits_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankInitDeposits_state->count = 0;
	current_xmachine_bank_holder = bank_BankInitMortgages_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankInitMortgages_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankInitMortgages_state->count = 0;
	current_xmachine_bank_holder = bank_BankLoop_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankLoop_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankLoop_state->count = 0;
	current_xmachine_bank_holder = bank_BankInitLoans_state->agents;
	while(current_xmachine_bank_holder)
	{
		temp_xmachine_bank_holder = current_xmachine_bank_holder->next;
		free_bank_agent(current_xmachine_bank_holder, bank_BankInitLoans_state);
		current_xmachine_bank_holder = temp_xmachine_bank_holder;
	}
	bank_BankInitLoans_state->count = 0;
...
void free_bank_states()
{
	...
	free(bank_BankCreditPolicyChecks_state);
	free(bank_BankUpdateDeposits_state);
	free(bank_BankCreditStart_state);
	free(bank_BankInitBalanceSheet_state);
	free(bank_BankInitDeposits_state);
	free(bank_BankInitMortgages_state);
	free(bank_BankLoop_state);
	free(bank_BankInitLoans_state);
 ...
 * \param loans_start Variable for the X-machine memory.
 * \param mortgages Variable for the X-machine memory.
...
void add_bank_agent(int id, int day_of_month_to_act, int day_of_week_to_act, int it_no, double total_assets, double loans, double loans_start, double mortgages, double deposits, double centralbank_debt, double equity, double liquidity, double revenues, double total_writeoffs, double interest_rate, double interests_accrued, double interests_paid, double dividends_paid, double total_dividends, double retained_earnings, double net_earnings, double total_costs)
{
	...
	current->total_assets = total_assets;
	current->loans = loans;
	current->loans_start = loans_start;
	current->mortgages = mortgages;
...
xmachine_memory_government * init_government_agent()
{
	...
	current->centralbank_income = 0.0;
...
void unittest_init_government_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_government->centralbank_income = 0.0;
...
{
	current_xmachine_government_holder = government_GovernmentUpdatePolicy_state->agents;
	while(current_xmachine_government_holder)
	{
		temp_xmachine_government_holder = current_xmachine_government_holder->next;
		free_government_agent(current_xmachine_government_holder, government_GovernmentUpdatePolicy_state);
		current_xmachine_government_holder = temp_xmachine_government_holder;
	}
	government_GovernmentUpdatePolicy_state->count = 0;
	...
	current_xmachine_government_holder = government_GovernmentCredit_state->agents;
	while(current_xmachine_government_holder)
	{
		temp_xmachine_government_holder = current_xmachine_government_holder->next;
		free_government_agent(current_xmachine_government_holder, government_GovernmentCredit_state);
		current_xmachine_government_holder = temp_xmachine_government_holder;
	}
	government_GovernmentCredit_state->count = 0;
	current_xmachine_government_holder = government_GovernmentLoop_state->agents;
	while(current_xmachine_government_holder)
	{
		temp_xmachine_government_holder = current_xmachine_government_holder->next;
		free_government_agent(current_xmachine_government_holder, government_GovernmentLoop_state);
		current_xmachine_government_holder = temp_xmachine_government_holder;
	}
	government_GovernmentLoop_state->count = 0;
...
{
	free(government_GovernmentUpdatePolicy_state);
	...
	free(government_GovernmentFiscalPolicy_state);
	free(government_GovernmentLabourStatus_state);
	free(government_GovernmentCredit_state);
	free(government_GovernmentLoop_state);
...
void add_government_agent(int id, int it_no, double average_wage, double unemployment_rate, int population_size, double debt, double equity, double liquidity, int day_of_month_to_act, int day_of_month_wages_paid, double capital_tax_rate, double labour_tax_rate, double labour_tax_income, double capital_tax_income, double gov_general_benefit_rate, double gov_unemployment_rate, double general_benefits, double unemployment_benefits, double earnings, double centralbank_income, double expenditures)
{
	...
	current->centralbank_income = centralbank_income;
...
xmachine_memory_centralbank * init_centralbank_agent()
{
	...
	current->liquidity_equityfund = 0.0;
	current->total_assets = 0.0;
	current->total_writeoffs = 0.0;
	current->interests_accrued = 0.0;
	current->revenues = 0.0;
...
void unittest_init_centralbank_agent()
{
	...
	CHECK_POINTER(current);
		...
		current_xmachine_centralbank->inflation_rate = 0.0;
		...
		current_xmachine_centralbank->liquidity_equityfund = 0.0;
...
{
	current_xmachine_centralbank_holder = centralbank_CentralBankUpdateInterestRate_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankUpdateInterestRate_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankUpdateInterestRate_state->count = 0;
	...
	current_xmachine_centralbank_holder = centralbank_CentralBankGovernmentRequests_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankGovernmentRequests_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankGovernmentRequests_state->count = 0;
	...
	current_xmachine_centralbank_holder = centralbank_CentralBankPolicy_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankPolicy_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankPolicy_state->count = 0;
	...
	current_xmachine_centralbank_holder = centralbank_CentralBankConsumptionPrices_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankConsumptionPrices_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankConsumptionPrices_state->count = 0;
	...
	current_xmachine_centralbank_holder = centralbank_CentralBankLoop_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankLoop_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankLoop_state->count = 0;
	current_xmachine_centralbank_holder = centralbank_CentralBankInitBalanceSheet_state->agents;
	while(current_xmachine_centralbank_holder)
	{
		temp_xmachine_centralbank_holder = current_xmachine_centralbank_holder->next;
		free_centralbank_agent(current_xmachine_centralbank_holder, centralbank_CentralBankInitBalanceSheet_state);
		current_xmachine_centralbank_holder = temp_xmachine_centralbank_holder;
	}
	centralbank_CentralBankInitBalanceSheet_state->count = 0;
 ...
 * \param liquidity_equityfund Variable for the X-machine memory.
 * \param total_assets Variable for the X-machine memory.
 * \param total_writeoffs Variable for the X-machine memory.
 * \param interests_accrued Variable for the X-machine memory.
 * \param revenues Variable for the X-machine memory.
...
void add_centralbank_agent(int id, int day_of_month_to_act, double unemployment_rate, double inflation_rate, double consumption_goods_prices[], int it_no, int day_of_week_to_act, transaction goods, double weekly_price_averages[], int day_of_month_wages_paid, double interest_rate, double liquidity, double loans_banks, double loans_government, double fiat_money, double equity, double liquidity_banks, double liquidity_government, double liquidity_equityfund, double total_assets, double total_writeoffs, double interests_accrued, double revenues, double net_earnings, double total_costs, transaction houses)
{
	...
	current->liquidity_equityfund = liquidity_equityfund;
	current->total_assets = total_assets;
	current->total_writeoffs = total_writeoffs;
	current->interests_accrued = interests_accrued;
	current->revenues = revenues;
...
void free_jpoffice_agents()
{
	...
	current_xmachine_jpoffice_holder = jpoffice_JPOfficeLoop_state->agents;
	while(current_xmachine_jpoffice_holder)
	{
		temp_xmachine_jpoffice_holder = current_xmachine_jpoffice_holder->next;
		free_jpoffice_agent(current_xmachine_jpoffice_holder, jpoffice_JPOfficeLoop_state);
		current_xmachine_jpoffice_holder = temp_xmachine_jpoffice_holder;
	}
	jpoffice_JPOfficeLoop_state->count = 0;
	current_xmachine_jpoffice_holder = jpoffice_JPOfficeInitEmployment_state->agents;
	while(current_xmachine_jpoffice_holder)
	{
		temp_xmachine_jpoffice_holder = current_xmachine_jpoffice_holder->next;
		free_jpoffice_agent(current_xmachine_jpoffice_holder, jpoffice_JPOfficeInitEmployment_state);
		current_xmachine_jpoffice_holder = temp_xmachine_jpoffice_holder;
	}
	jpoffice_JPOfficeInitEmployment_state->count = 0;
...
void free_jpoffice_states()
{
	...
	free(jpoffice_JPOfficeLoop_state);
	free(jpoffice_JPOfficeInitEmployment_state);
...
void free_reagency_agents()
{
	...
	current_xmachine_reagency_holder = reagency_REAgencyCreditPolicyChecks_state->agents;
	while(current_xmachine_reagency_holder)
	{
		temp_xmachine_reagency_holder = current_xmachine_reagency_holder->next;
		free_reagency_agent(current_xmachine_reagency_holder, reagency_REAgencyCreditPolicyChecks_state);
		current_xmachine_reagency_holder = temp_xmachine_reagency_holder;
	}
	reagency_REAgencyCreditPolicyChecks_state->count = 0;
...
void free_reagency_states()
{
	...
	free(reagency_REAgencyCreditPolicyChecks_state);
...
/** \fn void set_manager(int manager)
 * \brief Set manager memory variable for current X-machine.
 * \param manager New value for variable.
 */
void set_manager(int manager)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).manager = manager;
}

/** \fn int get_manager()
 * \brief Get manager memory variable from current X-machine.
 * \return Value for variable.
 */
int get_manager()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).manager;
...
void set_day_of_month_wages_paid(int day_of_month_wages_paid)
{
	...
	if(current_xmachine->xmachine_equityfund) (*current_xmachine->xmachine_equityfund).day_of_month_wages_paid = day_of_month_wages_paid;
	if(current_xmachine->xmachine_government) (*current_xmachine->xmachine_government).day_of_month_wages_paid = day_of_month_wages_paid;
	if(current_xmachine->xmachine_centralbank) (*current_xmachine->xmachine_centralbank).day_of_month_wages_paid = day_of_month_wages_paid;
}
...
int get_day_of_month_wages_paid()
{
	...
	if(current_xmachine->xmachine_equityfund) return (*current_xmachine->xmachine_equityfund).day_of_month_wages_paid;
	if(current_xmachine->xmachine_government) return (*current_xmachine->xmachine_government).day_of_month_wages_paid;
	if(current_xmachine->xmachine_centralbank) return (*current_xmachine->xmachine_centralbank).day_of_month_wages_paid;
...
/** \fn void set_capital_productivity(double capital_productivity)
 * \brief Set capital_productivity memory variable for current X-machine.
 * \param capital_productivity New value for variable.
 */
void set_capital_productivity(double capital_productivity)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).capital_productivity = capital_productivity;
}

/** \fn double get_capital_productivity()
 * \brief Get capital_productivity memory variable from current X-machine.
 * \return Value for variable.
 */
double get_capital_productivity()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).capital_productivity;
...
/** \fn void set_capital_goods(int capital_goods)
...
/** \fn void set_capital_goods_price(double capital_goods_price)
 * \brief Set capital_goods_price memory variable for current X-machine.
 * \param capital_goods_price New value for variable.
 */
void set_capital_goods_price(double capital_goods_price)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).capital_goods_price = capital_goods_price;
}

/** \fn double get_capital_goods_price()
 * \brief Get capital_goods_price memory variable from current X-machine.
 * \return Value for variable.
 */
double get_capital_goods_price()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).capital_goods_price;
...
/** \fn void set_expected_sales(int expected_sales)
 * \brief Set expected_sales memory variable for current X-machine.
 * \param expected_sales New value for variable.
 */
void set_expected_sales(int expected_sales)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).expected_sales = expected_sales;
}

/** \fn int get_expected_sales()
 * \brief Get expected_sales memory variable from current X-machine.
 * \return Value for variable.
 */
int get_expected_sales()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).expected_sales;
...
/** \fn void set_unit_cost(double unit_cost)
 * \brief Set unit_cost memory variable for current X-machine.
 * \param unit_cost New value for variable.
 */
void set_unit_cost(double unit_cost)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).unit_cost = unit_cost;
}

/** \fn double get_unit_cost()
 * \brief Get unit_cost memory variable from current X-machine.
 * \return Value for variable.
 */
double get_unit_cost()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).unit_cost;
...
/** \fn void set_isliquidshort(int isliquidshort)
 * \brief Set isliquidshort memory variable for current X-machine.
 * \param isliquidshort New value for variable.
 */
void set_isliquidshort(int isliquidshort)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).isliquidshort = isliquidshort;
}

/** \fn int get_isliquidshort()
 * \brief Get isliquidshort memory variable from current X-machine.
 * \return Value for variable.
 */
int get_isliquidshort()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).isliquidshort;
...
/** \fn void set_isilliquid(int isilliquid)
 * \brief Set isilliquid memory variable for current X-machine.
 * \param isilliquid New value for variable.
 */
void set_isilliquid(int isilliquid)
{
	if(current_xmachine->xmachine_firm) (*current_xmachine->xmachine_firm).isilliquid = isilliquid;
}

/** \fn int get_isilliquid()
 * \brief Get isilliquid memory variable from current X-machine.
 * \return Value for variable.
 */
int get_isilliquid()
{
	if(current_xmachine->xmachine_firm) return (*current_xmachine->xmachine_firm).isilliquid;
...
/** \fn void set_mall_budget(double mall_budget)
 * \brief Set mall_budget memory variable for current X-machine.
 * \param mall_budget New value for variable.
 */
void set_mall_budget(double mall_budget)
{
	if(current_xmachine->xmachine_household) (*current_xmachine->xmachine_household).mall_budget = mall_budget;
}

/** \fn double get_mall_budget()
 * \brief Get mall_budget memory variable from current X-machine.
 * \return Value for variable.
 */
double get_mall_budget()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).mall_budget;
...
/** \fn void set_ismanager(int ismanager)
 * \brief Set ismanager memory variable for current X-machine.
 * \param ismanager New value for variable.
 */
void set_ismanager(int ismanager)
{
	if(current_xmachine->xmachine_household) (*current_xmachine->xmachine_household).ismanager = ismanager;
}

/** \fn int get_ismanager()
 * \brief Get ismanager memory variable from current X-machine.
 * \return Value for variable.
 */
int get_ismanager()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).ismanager;
...
/** \fn void set_government_benefits(double government_benefits)
 * \brief Set government_benefits memory variable for current X-machine.
 * \param government_benefits New value for variable.
 */
void set_government_benefits(double government_benefits)
{
	if(current_xmachine->xmachine_household) (*current_xmachine->xmachine_household).government_benefits = government_benefits;
}

/** \fn double get_government_benefits()
 * \brief Get government_benefits memory variable from current X-machine.
 * \return Value for variable.
 */
double get_government_benefits()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).government_benefits;
...
/** \fn double get_previous_benefits()
 * \brief Get previous_benefits memory variable from current X-machine.
 * \return Value for variable.
 */
double * get_previous_benefits()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).previous_benefits;
...
/** \fn void set_expected_housing_payment(double expected_housing_payment)
 * \brief Set expected_housing_payment memory variable for current X-machine.
 * \param expected_housing_payment New value for variable.
 */
void set_expected_housing_payment(double expected_housing_payment)
{
	if(current_xmachine->xmachine_household) (*current_xmachine->xmachine_household).expected_housing_payment = expected_housing_payment;
}

/** \fn double get_expected_housing_payment()
 * \brief Get expected_housing_payment memory variable from current X-machine.
 * \return Value for variable.
 */
double get_expected_housing_payment()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).expected_housing_payment;
...
/** \fn void set_mortgage_choice(int mortgage_choice)
 * \brief Set mortgage_choice memory variable for current X-machine.
 * \param mortgage_choice New value for variable.
 */
void set_mortgage_choice(int mortgage_choice)
{
	if(current_xmachine->xmachine_household) (*current_xmachine->xmachine_household).mortgage_choice = mortgage_choice;
}

/** \fn int get_mortgage_choice()
 * \brief Get mortgage_choice memory variable from current X-machine.
 * \return Value for variable.
 */
int get_mortgage_choice()
{
	if(current_xmachine->xmachine_household) return (*current_xmachine->xmachine_household).mortgage_choice;
...
/** \fn void set_loans_start(double loans_start)
 * \brief Set loans_start memory variable for current X-machine.
 * \param loans_start New value for variable.
 */
void set_loans_start(double loans_start)
{
	if(current_xmachine->xmachine_bank) (*current_xmachine->xmachine_bank).loans_start = loans_start;
}

/** \fn double get_loans_start()
 * \brief Get loans_start memory variable from current X-machine.
 * \return Value for variable.
 */
double get_loans_start()
{
	if(current_xmachine->xmachine_bank) return (*current_xmachine->xmachine_bank).loans_start;
...
{
	if(current_xmachine->xmachine_bank) (*current_xmachine->xmachine_bank).interest_rate = interest_rate;
...
{
	if(current_xmachine->xmachine_bank) return (*current_xmachine->xmachine_bank).interest_rate;
...
/** \fn void set_centralbank_income(double centralbank_income)
 * \brief Set centralbank_income memory variable for current X-machine.
 * \param centralbank_income New value for variable.
 */
void set_centralbank_income(double centralbank_income)
{
	if(current_xmachine->xmachine_government) (*current_xmachine->xmachine_government).centralbank_income = centralbank_income;
}

/** \fn double get_centralbank_income()
 * \brief Get centralbank_income memory variable from current X-machine.
 * \return Value for variable.
 */
double get_centralbank_income()
{
	if(current_xmachine->xmachine_government) return (*current_xmachine->xmachine_government).centralbank_income;
...
{
	if(current_xmachine->xmachine_centralbank) (*current_xmachine->xmachine_centralbank).inflation_rate = inflation_rate;
}
...
{
	if(current_xmachine->xmachine_centralbank) return (*current_xmachine->xmachine_centralbank).inflation_rate;
...
/** \fn void set_liquidity_equityfund(double liquidity_equityfund)
 * \brief Set liquidity_equityfund memory variable for current X-machine.
 * \param liquidity_equityfund New value for variable.
 */
void set_liquidity_equityfund(double liquidity_equityfund)
{
	if(current_xmachine->xmachine_centralbank) (*current_xmachine->xmachine_centralbank).liquidity_equityfund = liquidity_equityfund;
}

/** \fn double get_liquidity_equityfund()
 * \brief Get liquidity_equityfund memory variable from current X-machine.
 * \return Value for variable.
 */
double get_liquidity_equityfund()
{
	if(current_xmachine->xmachine_centralbank) return (*current_xmachine->xmachine_centralbank).liquidity_equityfund;
...
void add_node(int node_id, double minx, double maxx, double miny, double maxy, double minz, double maxz)
{
	...
	current->agents = NULL;
	current->firm_bank_init_loans_messages = NULL;
	current->firm_bank_init_deposit_messages = NULL;
	current->firm_bank_update_deposit_messages = NULL;
	current->household_bank_init_mortgages_messages = NULL;
	current->household_bank_init_deposit_messages = NULL;
	current->household_bank_update_deposit_messages = NULL;
	current->bank_centralbank_init_debt_messages = NULL;
	current->bank_centralbank_init_deposit_messages = NULL;
	current->bank_centralbank_update_deposit_messages = NULL;
	current->fund_centralbank_init_deposit_messages = NULL;
	current->fund_centralbank_update_deposit_messages = NULL;
	current->gov_centralbank_init_deposit_messages = NULL;
	current->gov_centralbank_update_deposit_messages = NULL;
	current->interest_rate_messages = NULL;
	current->capital_tax_rate_messages = NULL;
	current->labour_tax_rate_messages = NULL;
	current->household_jpoffice_id_messages = NULL;
	current->jpoffice_household_employer_messages = NULL;
	current->firm_jpoffice_id_messages = NULL;
	current->jpoffice_firm_employee_messages = NULL;
	...
	current->firm_household_wage_payment_messages = NULL;
	current->firm_bank_loan_request_1_messages = NULL;
	current->firm_bank_loan_request_2_messages = NULL;
	current->bank_firm_loan_acknowledge_1_messages = NULL;
	current->bank_firm_loan_acknowledge_2_messages = NULL;
	current->bank_centralbank_debt_request_messages = NULL;
	...
	current->firm_bank_interest_on_loan_messages = NULL;
	current->loan_writeoff_messages = NULL;
	current->firm_bank_insolvent_account_messages = NULL;
	current->firm_net_profit_messages = NULL;
	current->bank_net_profit_messages = NULL;
	current->bank_centralbank_interest_payment_messages = NULL;
	current->bank_centralbank_debt_payment_messages = NULL;
	current->gov_centralbank_debt_request_messages = NULL;
	current->gov_centralbank_debt_payment_messages = NULL;
	current->centralbank_government_profit_messages = NULL;
	...
	current->bank_reagency_credibility_messages = NULL;
...
void clean_up(int code)
{
	...
	rc = MB_Delete(&b_firm_bank_init_loans);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_init_loans' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_bank_init_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_init_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_household_bank_init_mortgages);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'household_bank_init_mortgages' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_household_bank_init_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'household_bank_init_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_init_debt);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_init_debt' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_init_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_init_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_update_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_update_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_fund_centralbank_init_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'fund_centralbank_init_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_fund_centralbank_update_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'fund_centralbank_update_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_gov_centralbank_init_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'gov_centralbank_init_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_gov_centralbank_update_deposit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'gov_centralbank_update_deposit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_household_jpoffice_id);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'household_jpoffice_id' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_jpoffice_household_employer);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'jpoffice_household_employer' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_jpoffice_id);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_jpoffice_id' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_jpoffice_firm_employee);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'jpoffice_firm_employee' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_household_wage_payment);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_household_wage_payment' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_bank_loan_request_1);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_loan_request_1' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_bank_loan_request_2);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_loan_request_2' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_firm_loan_acknowledge_1);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_firm_loan_acknowledge_1' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_firm_loan_acknowledge_2);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_firm_loan_acknowledge_2' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_debt_request);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_debt_request' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_bank_interest_on_loan);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_interest_on_loan' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_firm_bank_insolvent_account);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'firm_bank_insolvent_account' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_interest_payment);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_interest_payment' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_centralbank_debt_payment);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_centralbank_debt_payment' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_gov_centralbank_debt_request);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'gov_centralbank_debt_request' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_gov_centralbank_debt_payment);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'gov_centralbank_debt_payment' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_centralbank_government_profit);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'centralbank_government_profit' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
	...
	rc = MB_Delete(&b_bank_reagency_credibility);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'bank_reagency_credibility' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
...
void copy_buyer_array(buyer_array * from, buyer_array * to)
{
	...
	for(i = 0; i < (*from).size; i++)
	{
		add_buyer(to, (*from).array[i].id, (*from).array[i].budget);
	}
}

/** \fn void add_buyer(buyer_array * array, int id, double budget)
...
void copy_seller_array(seller_array * from, seller_array * to)
{
	...
	for(i = 0; i < (*from).size; i++)
	{
		add_seller(to, (*from).array[i].id, (*from).array[i].price, (*from).array[i].inventory, (*from).array[i].inv_price);
	}
}

/** \fn void add_seller(seller_array * array, int id, double price, int inventory, double inv_price)
...
void add_seller(seller_array * array,  int id,  double price,  int inventory,  double inv_price)
{
	...
	(*array).array[(*array).size].inv_price = inv_price;
...
void copy_mortgage_array(mortgage_array * from, mortgage_array * to)
{
	...
	for(i = 0; i < (*from).size; i++)
	{
		add_mortgage(to, (*from).array[i].bank_id, (*from).array[i].principal, (*from).array[i].quarters_left, (*from).array[i].quarterly_interest, (*from).array[i].quarterly_principal, (*from).array[i].interestrate, (*from).array[i].mtype);
	}
}

/** \fn void add_mortgage(mortgage_array * array, int bank_id, double principal, int quarters_left, double quarterly_interest, double quarterly_principal, double interestrate, int mtype)
...
void add_mortgage(mortgage_array * array,  int bank_id,  double principal,  int quarters_left,  double quarterly_interest,  double quarterly_principal,  double interestrate,  int mtype)
{
	...
	(*array).array[(*array).size].interestrate = interestrate;
	(*array).array[(*array).size].mtype = mtype;
...
void copy_hbuyer_array(hbuyer_array * from, hbuyer_array * to)
{
	...
	for(i = 0; i < (*from).size; i++)
	{
		add_hbuyer(to, (*from).array[i].buyer_id, (*from).array[i].bank_id, (*from).array[i].liquidity, (*from).array[i].quarterly_income, (*from).array[i].quarterly_mortgage_paid, (*from).array[i].choice);
	}
}

/** \fn void add_hbuyer(hbuyer_array * array, int buyer_id, int bank_id, double liquidity, double quarterly_income, double quarterly_mortgage_paid, int choice)
...
void add_hbuyer(hbuyer_array * array,  int buyer_id,  int bank_id,  double liquidity,  double quarterly_income,  double quarterly_mortgage_paid,  int choice)
{
	...
	(*array).array[(*array).size].choice = choice;
...
void copy_hseller_array(hseller_array * from, hseller_array * to)
{
	...
	for(i = 0; i < (*from).size; i++)
	{
		add_hseller(to, (*from).array[i].seller_id, (*from).array[i].price, (*from).array[i].quantity, (*from).array[i].type);
	}
}

/** \fn void add_hseller(hseller_array * array, int seller_id, double price, int quantity, int type)
...
{
	return FLAME_environment_variable_resume_mode;
}
int FLAME_get_environment_variable_print_debug_mode()
{
	return FLAME_environment_variable_print_debug_mode;
}
int FLAME_get_environment_variable_warning_mode()
{
	return FLAME_environment_variable_warning_mode;
}
int FLAME_get_environment_variable_data_collection_mode()
{
	return FLAME_environment_variable_data_collection_mode;
}
int FLAME_get_environment_variable_collect_household_data()
{
	return FLAME_environment_variable_collect_household_data;
}
int FLAME_get_environment_variable_collect_firm_data()
{
	return FLAME_environment_variable_collect_firm_data;
}
